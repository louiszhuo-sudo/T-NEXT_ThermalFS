        ...
        series: [
            {
                ...
                // itemStyle: {
                //     color: (params) => {
                //         const xValue = params.name; // 取得 xAxis 的名稱 (例如 "16:00")

                //         // 1. 檢查格式是否為 HH:mm (簡單正則判斷)
                //         const timeRegex = /^([01]\d|2[0-3]):([0-5]\d)$/;
                //         if (!timeRegex.test(xValue)) {
                //             return '#91BADB'; // 格式不符，固定顯示藍色
                //         }

                //         // 2. 解析小時
                //         const hour = parseInt(xValue.split(':')[0], 10);

                //         // 3. 判斷時間區間 (18:00 ~ 06:00)
                //         // 邏輯：小時 >= 18 或者 小時 < 6
                //         if (hour >= 18 || hour < 6) {
                //             return '#898989'; // 深灰色
                //         } else {
