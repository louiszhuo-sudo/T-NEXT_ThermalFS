            ...
            try {
                ...
                if (data?.feature) {
                    ...
                    if (feature === 'initial') {
                        ...
                        const output: any[] = [];
                        // 2. 提前獲取資料列，避免在迴圈中重複計算 content[1]
                        for (var z = 1; z <= 2; z++) {
                            const rows = content[z] || [];

                            for (let i = 0; i < rows.length; i++) {
                                const row = rows[i];
                                const targetKey = row[0];
                                // 3. 只 get 一次，存入變數
                                const targetConfig = indexMap.get(targetKey);
                                if (targetConfig) {
                                    // 4. 移除 console.log，只有在開發偵錯且非頻繁刷新時才開啟
                                    if (targetConfig.type === 'roi') {
                                        savetreeview[targetConfig.path[0]].childNodes[targetConfig.path[1]].childNodes[targetConfig.path[2]].data.roi_alarmStatus = row[1]
                                        savetreeview[targetConfig.path[0]].childNodes[targetConfig.path[1]].childNodes[targetConfig.path[2]].data.roi_maxTemperature = row[2]
                                        savetreeview[targetConfig.path[0]].childNodes[targetConfig.path[1]].childNodes[targetConfig.path[2]].data.roi_alarmContent = row[3]
                                    }
