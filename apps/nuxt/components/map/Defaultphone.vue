<template>
    <div class="item-card-content" id="map-top-b" style="height: 100%; width: 100%;z-index: 0;" ref="mapcanvas">
        ...
        <v-dialog v-model="state.inputRoiNameWindow" width="auto" persistent>
            <v-card max-width="400" prepend-icon="mdi-update" title="確定新增嗎?">
...
<v-switch color="primary" v-model="state.tempAlarmSwitch" :label="`警報開關: ${state.tempAlarmSwitch ? '開' : '關'}`"
    hide-details inset></v-switch>
</template> -->
                <template v-slot:actions>
                    <v-btn text="取消" @click="inputRoiName('delete')"></v-btn>
                    <v-btn text="送出" @click="inputRoiName('save')"></v-btn>
                </template>
            </v-card>
        </v-dialog>
...
const message12 = ref('')
const aispotControl = ref('')
const roiMorphControl = ref('')
var map = null
...
//  測試子組件使用父組件方法
const emit = defineEmits(['dataFromChild', 'messageEvent', 'backbtnfn', 'startThermalCapture'])
setTimeout(() => {
    sendDataToParent()
}, 1000)
...
const testp = (map) => {
    ...
    // 利用原始 SVG path 綁定事件（這是關鍵）
    rect.once('add', () => {
        const path = rect._path;

        // pointerdown
        L.DomEvent.on(path, 'pointerdown', (e) => {
            isDragging = true;
            startLatLng = map.mouseEventToLatLng(e);
            startBounds = rect.getBounds();
            map.dragging.disable(); // 關閉地圖拖曳
            L.DomEvent.stopPropagation(e);
            L.DomEvent.preventDefault(e);
        });

        // pointermove
        L.DomEvent.on(map._container, 'pointermove', (e) => {
            if (!isDragging) return;
            const curr = map.mouseEventToLatLng(e);
            const dx = curr.lng - startLatLng.lng;
            const dy = curr.lat - startLatLng.lat;

            const newBounds = L.latLngBounds(
                [
                    startBounds.getSouthWest().lat + dy,
                    startBounds.getSouthWest().lng + dx
                ],
                [
                    startBounds.getNorthEast().lat + dy,
                    startBounds.getNorthEast().lng + dx
                ]
            );
        ...
        // pointerup
        L.DomEvent.on(map._container, 'pointerup', (e) => {
            if (isDragging) {
                isDragging = false;
                map.dragging.enable();
            }
...
const leafletJsInit = () => {
    ...
    map.on('mousemove', (e) => {
        ...
        if (state.tempMouseDownScope !== null) {
            ...
            // console.log("1", JSON.stringify(state.tempMouseDownScopelnglat), "2", JSON.stringify(tmepBounds), "3:", JSON.stringify(tempmarker));
            ...
            //     temp3.lng - x
            // ]
            // console.log(scopewidth, scopehight, temp[0][1].lat, state.imgHeight + scopehight);
            if (temp[0][1].lng < 0) {
                temp[0][0].lng = 0
                temp[0][1].lng = 0
            ...
            // console.log('scope', markerNew, temp);
...
const roiMorphsendItemstemp = (data, bounds, LatLngs) => {
    ...
    if (findIndex === -1) {
        ...
        // };
        // // 建立並加入地圖
        // const scope = L.polygon(bounds, scopeStyle).addTo(map);
        // scope.bringToFront();     // 讓它在最上層（可選）
...
const onCanvasDown = (e) => {
    console.log('onCanvasDown', e, state.spotStatusAdd);
    ...
    if (state.spotStatusAdd) {
        ...
        // console.log('state.selectMainGroupValue', state.selectMainGroupValue);
...
const pixiaddScope = (data, name, tempPointID) => {
    ...
    var myIcon = L.divIcon({ html: myIconHtml });
    var mky = data.position_point_A.y < data.position_point_B.y ? data.position_point_A.y : data.position_point_B.y
    var mkx = data.position_point_A.x > data.position_point_B.x ? data.position_point_A.x : data.position_point_B.x
    var marker = L.marker([mky * height, mkx * width], { icon: myIcon, objectType: 'icon', opacity, draggable: false, id: `scope${data.number}` }).addTo(group);
    ...
    var rectangular = L.rectangle(bounds, { color: "#fff00", weight: 2, fillOpacity: 0, objectType: 'rectangle', id: `scope${data.number}` })
    // 利用原始 SVG path 綁定事件（這是關鍵）
    // 狀態變數
    ...
    if (true) {
        ...
        rectangular.once('add', () => {
            const path = rectangular._path;

            // pointerdown
            L.DomEvent.on(path, 'click', (e) => {
                state.scopetouch = true
            })
            L.DomEvent.on(path, 'pointerdown', (e) => {
                if (!state.scopetouch) return;
                startLatLng = map.mouseEventToLatLng(e);
                startBounds = rectangular.getBounds();
                map.dragging.disable(); // 關閉地圖拖曳
                L.DomEvent.stopPropagation(e);
                L.DomEvent.preventDefault(e);
                ...
                tmepBounds = rectangular.getLatLngs()
                tempmarker = marker.getLatLng()
                state.tempMouseDownScopeObject = {}
                state.tempMouseDownScopeObject.tmepBounds = rectangular.getLatLngs()
                ...
                state.tempMouseDownScopeObject.target = rectangular
            ...
            L.DomEvent.on(map._container, 'pointermove', (e) => {
                ...
                rectangular.setBounds(newBounds);
                rectangleBorder1.setBounds(newBounds);
                rectangleBorder2.setBounds(newBounds);
                let markerNew = [
                    newBounds._northEast.lat, newBounds._northEast.lng
                ]
                console.log('markerNew', newBounds)
...
const runRTC = (id, url, videoType) => {
    ...
    const onTrack = (evt) => {
        ...
        // message12.value.innerText = '完成RTC連線' + JSON.stringify(evt.streams[0])
...
const timeOUT01 = ref(null)
const timeOUT02 = ref(null)
const startThermalCapture = async (videoId = 'video', fps = 15) => {
    const video = document.getElementById(videoId);
    if (!video) return console.error('找不到指定 video tag');
    ...
    // 啟動 Worker
    worker.value = new Worker('/worker/thermalWorker.js');
    worker.value.postMessage({ type: 'init', width: canvas.width, height: canvas.height });
    const run = () => {
        ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
        const imgData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        worker.value.postMessage({ type: 'frame', image: imgData }, [imgData.data.buffer]); // 零拷貝傳輸
    }
    worker.value.onmessage = (e) => {
        if (e.data.type === 'result') {
            console.log(e.data);
            emit('startThermalCapture', e.data.payload);
            if (timeOUT01.value !== null) {
                clearTimeout(timeOUT01.value)
                timeOUT01.value = null
            }
            timeOUT01.value = setTimeout(() => {
                run()
            }, 100)
        }
    };
    if (timeOUT02.value !== null) {
        clearTimeout(timeOUT02.value)
        timeOUT02.value = null
    }
    timeOUT02.value = setTimeout(() => {
        run()
    }, 500)
...
onMounted(() => {
    ...
    startThermalCapture(`video${camType}${camID}-${state.randomID}`, 2)
    setTimeout(() => {
        worker.value.terminate()
        worker.value = null
    }, 5 * 1000)
