                                                            ...
                                                            <template v-if="item.data.roi_mainGroup_type === 1">
                                                                <v-menu activator="parent">
                                                                    <v-list>
                                                                        ...
                                                                        <v-list-item
                                                                            v-for="meunItem1 in [{ title: '修改名稱', value: 8 }, { title: '修改二級警報溫度', value: 2 }, { title: '刪除次群組', value: 12 }]"
...
watch(
    ...
    (e, prevData) => {
        ...
        // setTimeout(() => {
        state.treeviews = e
        state.roiMainGroups = transformData(e)
        // }, 0);
        // }
    }
)
const updateSubGroup = (e) => {
    var data = state.treeviews.find((item) => {
...
const vCheckboxindeterminatelevel1 = (e) => {
    let hasOne = false;  // 用来跟踪是否有返回值为1的情况
    let allZero = true;  // 用来跟踪是否所有值都为0
