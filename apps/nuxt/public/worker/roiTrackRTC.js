...
const runWs3 = (url) => {
    console.log('run 114');
    // ws3 = new WebSocket('ws://192.168.0.173:8702/')
    ws3 = new WebSocket('ws://192.168.0.114:8702/')
    ws3.onopen = () => {
        console.log('cameraphoto worker run websoket 03');
        // 測試
        // setTimeout(() => {
    ...
    var timeouts = [1000, 1100, 1200, 2000, 3000]
    var r = false
    setInterval(() => {
        r = true;
...
// 方法
const getRoiData = async (parameter) => {
    // console.log('getRoiData', parameter);
    ...
    for (var z = 0; z < oldDataLen; z++) {
        try {
            ...
            if (selectRoi1 === undefined) {
                ...
                oldData = oldData.filter(obj => obj.info.number !== oldData[z]?.info.number);
            ...
            console.log('判斷ROI 是否遭到刪除', oldData[z]);
        }
    }
    // 判斷
        ...
        try {
            ...
            if (selectRoi !== undefined) {
                ...
                if (JSON.stringify(selectRoi.info.points) === JSON.stringify(data[i].points)
                    ...
                    pixiBlobfind12 = null
                    // console.log('changeMapBlobDATA', output);
                    self.postMessage({
                    ...
                    // console.log('changeMapBlob', output);
                    self.postMessage({
                        type: 'changeMapBlob',
                        parameter: output
                    });
            ...
            console.log('判斷ROI 是否遭到刪除', 'data[i].number', data[i]);
        }
    }
    // if (objectName === "blob") {
